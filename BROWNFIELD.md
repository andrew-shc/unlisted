...
- 2026-07-09: Created `GREENFIELD/restir/` — 8 canonical entry-point scripts that drive the DigitalTwinCatalog PBIR runtime, adding WandB logging throughout. These are the user-facing scripts; they import from `DigitalTwinCatalog/neural_pbir/pbir/` (opt.py, datasets.py, models, irtk) via `sys.path.insert(0, ...)`. DigitalTwinCatalog's own `run_restir.py` and `run_restir_fwd.py` remain in place (they're still importable by these scripts) but `GREENFIELD/restir/run_restir.py` is now the canonical entry point. All scripts use `REPO_ROOT = Path(__file__).resolve().parents[2]` for dynamic path detection. See `GREENFIELD/restir/AGENTS.md` for the directory layout and gotchas.
