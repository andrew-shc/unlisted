...
PYBIND11_MODULE(psdr_jit, m) {
    ...
    py::class_<PathReSTIR, Integrator>(m, "PathReSTIR")
        ...
        .def("render_restir_path", &PathReSTIR::render_restir_path,
             "scene"_a, "sensor_id"_a = 0, "seed"_a = -1,
             "Primal: ReSTIR DI at depth 0 + MIS path tracing for depth 1+. "
             "Caches reservoir state for the subsequent renderD call.")
        .def("renderD", &PathReSTIR::renderD,
             "scene"_a, "sensor_id"_a = 0, "seed"_a = -1, "batch_pix"_a = -1,
             "Differentiable render: depth-0 detached-adjoint ReSTIR gradient "
             "+ depth 1+ standard differentiable path + primary edges.")
        .def_readwrite("max_depth",      &PathReSTIR::m_max_depth)
        .def_readwrite("n_candidates",   &PathReSTIR::m_n_candidates)
        .def_readwrite("n_neighbors",    &PathReSTIR::m_n_neighbors)
        .def_readwrite("spatial_radius", &PathReSTIR::m_spatial_radius)
        .def_readwrite("hide_emitters",  &PathReSTIR::m_hide_emitters);

    py::class_<DirectReSTIR, Integrator>(m, "DirectReSTIR")
        .def(py::init<int, int, int>(),
             "n_candidates"_a = 4, "n_neighbors"_a = 5, "spatial_radius"_a = 30)
        .def("render_restir", &DirectReSTIR::render_restir,
             "scene"_a, "sensor_id"_a = 0, "seed"_a = -1,
             "Two-pass ReSTIR DI: WRS candidate selection + parallel spatial reuse. "
             "Returns one primal sample per pixel. Average multiple calls for higher spp.")
        .def_readwrite("n_candidates",   &DirectReSTIR::m_n_candidates,
                       "M: light candidates generated per pixel (higher → lower variance, more cost)")
        .def_readwrite("n_neighbors",    &DirectReSTIR::m_n_neighbors,
                       "K: spatial neighbours to reuse from")
        .def_readwrite("spatial_radius", &DirectReSTIR::m_spatial_radius,
                       "Pixel search radius for neighbour selection")
        .def_readwrite("hide_emitters",  &DirectReSTIR::m_hide_emitters);
