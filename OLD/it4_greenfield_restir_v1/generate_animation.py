...
scene.set('luminaire', mesh_from_obj(f'{CBOX}/cbox_luminaire.obj',
    radiance=[20., 20., 8.],
    to_world=torch.tensor([[1.,0.,0.,0.],[0.,1.,0.,-0.5],[0.,0.,1.,0.],[0.,0.,0.,1.]]),
...
render_opt = Renderer('psdr_jit', render_options={'spp': 16, 'sppe': 16, 'sppse': 8,  'npass': 1, 'log_level': 0})
render_vis = Renderer('psdr_jit', render_options={'spp': 64, 'sppe': 0,  'sppse': 0,  'npass': 1, 'log_level': 0})

# ── Target render ──────────────────────────────────────────────────────────────
print("Rendering target…")
target = render_vis(scene, sensor_ids=[0])[0]  # (512,512,3) on cuda
write_image('shape_target_anim.exr', target)
...
for it in range(num_iters):
    ...
    if it % save_every == 0:
        ...
        print(f"  iter {it:3d}  L1={image_loss.item():.5f}  mean_Y={mean_ys[-1]:.1f}  (GT={V_gt[:,1].mean():.1f})")
