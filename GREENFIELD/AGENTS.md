...
Everything below (architecture, entry points, ReSTIR integration details, WandB logging, known issues, results, run protocol, file index, next steps) was folded in from the former root-level `REF.md` on 2026-07-08 — it's the detailed engineering reference; the sections above are the one-paragraph "why we're doing this" anchor.
...
| Script | Purpose |
|---|---|
| `GREENFIELD/nbir_setup_refined.py` / `.ipynb` | Standard Neural-PBIR pipeline, all 7 scenes |
| `GREENFIELD/nbir_restir_pipeline.py` | ReSTIR pipeline, teapot_scene001 only (initial eval) |
| `GREENFIELD/test_shape_opt_cornell.py` | Cornell box benchmark: PathTracer vs PathReSTIR |
...
metrology_ir/
  ...
  AGENTS.md                          this file (merged from former REF.md)
