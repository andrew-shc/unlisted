...
Why is PathReSTIR::renderC() NOT used for the forward pass?
  PathReSTIR::renderC() is a direct-illumination-only stub (needed for
  silhouette/boundary edge gradients). Using it for the forward loss would
  compare a direct-only render against a full-path target and bias the gradient.
  Therefore the forward pass always uses a standard PathTracer integrator
  (full-path, unbiased), while only the backward gradient pass uses PathReSTIR.
...
Usage:
  ...
  PYTHONUNBUFFERED=1 python GREENFIELD/restir/cornell_benchmark.py
  PYTHONUNBUFFERED=1 python GREENFIELD/restir/cornell_benchmark.py --iters 10

Output:
  ASSETS/restir/cornell_benchmark/
    results.json       — numerical results for all 3 methods
    target.exr          — ground-truth render
    cornell_benchmark.gif — animation of the optimization
"""
...
RESTIR_SPP = {'spp':  1, 'sppe':  1, 'sppse': 1}
...
def main(num_iters=50):
    out_dir   = REPO_ROOT / 'ASSETS' / 'restir' / 'cornell_benchmark'
    out_dir.mkdir(parents=True, exist_ok=True)
    gif_path  = out_dir / 'cornell_benchmark.gif'
    ...
    if _HAS_WANDB:
        wandb.init(
            ...
            name=f"cornell_benchmark/{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}",
            config={
                "method":          "cornell_benchmark",
                "num_iters":       num_iters,
                "save_every":      save_every,
                "standard_spp":     OPT_SPP['spp'],
                "restir_spp":      RESTIR_SPP['spp'],
                "restir_cand4":    RESTIR_PARAMS_4['n_candidates'],
                "restir_cand8":    RESTIR_PARAMS_8['n_candidates'],
                "lr_standard":      5.0,
                "lr_restir":        3.0,
            },
        )
    ...
    if _HAS_WANDB and wandb.run:
        wandb.log({
            ...
            "summary/restir_cand4_final_l1":   losses_r4[-1],
            "summary/restir_cand4_final_y":    final_y_r4,
            "summary/restir_cand4_fps":        fps_r4,
            "summary/restir_cand8_final_l1":   losses_r8[-1],
            "summary/restir_cand8_final_y":    final_y_r8,
            "summary/restir_cand8_fps":        fps_r8,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Cornell box shape optimization benchmark')
    parser.add_argument('--iters', type=int, default=50,
                        help='Number of optimization iterations')
    args = parser.parse_args()
    main(num_iters=args.iters)
